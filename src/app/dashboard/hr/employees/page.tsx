...
const employees = [
  { name: "Amira Benali",     position: "HR Manager",        contract: "CDI", status: "Active",   dept: "HR",        joined: "Mar 2021" },
  { name: "Nour Hamdi",       position: "HR Coordinator",    contract: "CDI", status: "Active",   dept: "HR",        joined: "Jul 2022" },
  { name: "Yasmine Chaabane", position: "Recruiter",         contract: "CDD", status: "Active",   dept: "HR",        joined: "Jan 2023" },
  { name: "Ines Jebali",      position: "Payroll Officer",   contract: "CDI", status: "On Leave", dept: "HR",        joined: "Sep 2020" },
  { name: "Khalil Mansouri",  position: "Marketing Manager", contract: "CDI", status: "Active",   dept: "Marketing", joined: "Feb 2020" },
  { name: "Tarek Jebali",     position: "Content Creator",   contract: "CDD", status: "Active",   dept: "Marketing", joined: "Oct 2023" },
  { name: "Sarra Tlili",      position: "Sales Executive",   contract: "CDI", status: "Active",   dept: "Sales",     joined: "May 2021" },
  { name: "Fares Dridi",      position: "Sales Manager",     contract: "CDI", status: "Inactive", dept: "Sales",     joined: "Nov 2019" },
];
...
export default function HREmployees() {
  ...
  const statusLabel = (s: string) =>
    s === "Active" ? t("active") : s === "On Leave" ? t("onLeave") : t("inactive");
  const statusStyle = (s: string) =>
    s === "Active" ? "bg-emerald-500/15 text-emerald-400" :
    s === "On Leave" ? "bg-amber-500/15 text-amber-400" : "bg-red-500/15 text-red-400";
  const dotStyle = (s: string) =>
    s === "Active" ? "bg-emerald-400" : s === "On Leave" ? "bg-amber-400" : "bg-red-400";
      ...
      <DashboardLayout>
        <div className="min-h-screen bg-gray-100 dark:bg-[#060a0f] text-gray-900 dark:text-white font-mono p-6 space-y-6 transition-colors duration-300">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold tracking-tight leading-none">
                {t("employeeManagement").split(" ")[0]} <span className="text-emerald-400">{t("employeeManagement").split(" ").slice(1).join(" ")}</span>
              </h1>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
            {[
              { label: t("totalEmployeesKpiShort"), value: String(employees.length),                                         sub: t("allDepartments"),    icon: <Users size={14} />,     iconBg: "bg-emerald-500/10 text-emerald-400" },
              ...
              { label: t("cdiContracts"),           value: String(employees.filter(e => e.contract === "CDI").length),        sub: t("permanentStaff"),    icon: <FileText size={14} />,  iconBg: "bg-purple-500/10 text-purple-400" },
            ].map((s, i) => (
          ...
          <div className={`${card} overflow-hidden`}>
            ...
            <div className="grid px-6 py-3 text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-600 border-b border-gray-100 dark:border-white/[0.04]"
              style={{ gridTemplateColumns: "2fr 2fr 1fr 1.2fr 1fr 0.8fr" }}>
              <span>{t("employee")}</span><span>{t("position")}</span><span>{t("dept")}</span>
              <span>{t("contract")}</span><span>{t("status")}</span><span>{t("joined")}</span>
            </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((emp, i) => (
                <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500">{emp.dept}</p>
                  <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-bold w-fit ${CONTRACT_BADGE[emp.contract]}`}>{emp.contract}</span>
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold w-fit ${statusStyle(emp.status)}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${dotStyle(emp.status)}`} />
